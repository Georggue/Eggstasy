...
{
    public UnityAction PlaybackFinished = delegate {  };
    ...
    void Update()
    {
        if (audioSource.isPlaying)
        {
            ...
            for (int i = 0; i < nBand; i++)
            {
                float specVal = (float) System.Math.Max(-100.0f, 20.0f * (float) System.Math.Log10(averages[i]) + 160);
            ...
            for (int i = tempopd / 2; i < System.Math.Min(colmax, 2 * tempopd); ++i)
            {
                ...
                float score = onset + scorefun[(now - i + colmax) % colmax] -
                              alph * (float) System.Math.Pow(System.Math.Log((float) i / (float) tempopd), 2);
